...
static float LMSFilter(int64_t time, int v)
{
    ...
    ALOGD("A=%f, B=%f, C=%f, D=%f, E=%f", A,B,C,D,E);
    ALOGD("%lld  %d  %f  %f", time, v, f, a);
...
static int open_akm()
{
    ...
    if (sAkmFD <= 0) {
        ...
        ALOGD("%s, fd=%d", __PRETTY_FUNCTION__, sAkmFD);
        ALOGE_IF(sAkmFD<0, "Couldn't open %s (%s)",
...
static void close_akm()
{
    ...
    if (sAkmFD > 0) {
        ALOGE("%s, fd=%d", __PRETTY_FUNCTION__, sAkmFD);
        close(sAkmFD);
        sAkmFD = -1;
    }
}
...
static void enable_disable(int fd, uint32_t sensors, uint32_t mask)
{
    ...
    if (mask & SENSORS_PROXIMITY) {
        ...
        if (ioctl(fd, ECS_IOCTL_APP_SET_PFLAG, &flags) < 0) {
            ALOGE("ECS_IOCTL_APP_SET_PFLAG error (%s)", strerror(errno));
        }
    }
#endif	
...
static native_handle_t* sensors_control_open_data_source(struct sensors_control_device_t *dev)
{
    ALOGI("sensors_control_open_data_source");
            ...
            int handle, int enabled)
{
    ...
    ALOGD("%s active=%08x sensor=%08x new_sensors=%x changed=%x", __FUNCTION__,
        active, sensor, new_sensors, changed);
    ALOGI("sensors_control_activate");
...
static int sensors_control_device_close(struct hw_device_t *dev)
{
    ...
    ALOGD("sensors_control_device_close");
    free(ctx);
    return 0;
}
...
static int sensors_data_device_close(struct hw_device_t *dev)
{
    ...
    ALOGD("sensors_data_device_close");
    free(ctx);
    return 0;
}
...
static int sensors_data_data_open(struct sensors_data_device_t *dev, native_handle_t* nh)
{
    ...
    sPendingSensors = 0;
    sInputFD = dup(fd);
    ALOGD("sensors_data_open: fd = %d", sInputFD);
    return 0;
}
...
static int pick_sensor(sensors_data_t* values)
{
    ...
    ALOGD("No sensor to return!!! sPendingSensors=%08x", sPendingSensors);
    ...
    while (1) {
        ...
        if (nread == sizeof(event)) {
            ...
            if (event.type == EV_ABS) {
                ALOGD("type: %d code: %d value: %-5d time: %ds",
